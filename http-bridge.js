...
const DONE_PREFIX = '\x00DONE:';
const DONE_SUFFIX = '\x00';
...
function ensureBuf(id) {
  if (!outputBuffers.has(id)) outputBuffers.set(id, { buf: '', waiters: [] });
  return outputBuffers.get(id);
}

function appendOutput(id, data) {
  if (!outputBuffers.has(id)) return; // session not MCP-managed
  ...
  for (let i = entry.waiters.length - 1; i >= 0; i--) {
    ...
    const markerIdx = entry.buf.indexOf(w.marker);
    if (markerIdx !== -1) {
      const output = entry.buf.slice(0, markerIdx);
      // Parse exit code from marker, e.g. \x00DONE:0\x00
      const after = entry.buf.slice(markerIdx + w.marker.length);
      const codeMatch = after.match(/^(\d+)\x00/);
      const exitCode = codeMatch ? parseInt(codeMatch[1], 10) : 0;
      // Consume up to and including the exit-code suffix
      const consumed = markerIdx + w.marker.length + (codeMatch ? codeMatch[0].length : 0);
...
function waitForDone(id, marker, timeoutMs) {
  return new Promise((resolve, reject) => {
    ...
    // Already in buffer?
    const idx = entry.buf.indexOf(marker);
    if (idx !== -1) {
      const output = entry.buf.slice(0, idx);
      const after  = entry.buf.slice(idx + marker.length);
      const codeMatch = after.match(/^(\d+)\x00/);
      const exitCode = codeMatch ? parseInt(codeMatch[1], 10) : 0;
      const consumed = idx + marker.length + (codeMatch ? codeMatch[0].length : 0);
      entry.buf = entry.buf.slice(consumed);
...
async function handleRequest(req, res) {
  ...
  if (req.method === 'GET' && url === '/sessions') {
    ...
    for (const [id] of outputBuffers) {
      sessions.push({ id, type: _terminals.has(id) ? 'pty' : 'serial' });
    }
